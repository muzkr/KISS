...
// PF9
#define CSN_GPIOx GPIOF
#define CSN_PIN LL_GPIO_PIN_9
// PB8
#define SCL_GPIOx GPIOB
#define SCL_PIN LL_GPIO_PIN_8
// PB9
#define SDA_GPIOx GPIOB
...
{
    LL_GPIO_ResetOutputPin(CSN_GPIOx, CSN_PIN);
}

static inline void CS_Release()
{
    LL_GPIO_SetOutputPin(CSN_GPIOx, CSN_PIN);
}

static inline void SCL_Reset()
{
    LL_GPIO_ResetOutputPin(SCL_GPIOx, SCL_PIN);
}

static inline void SCL_Set()
{
    LL_GPIO_SetOutputPin(SCL_GPIOx, SCL_PIN);
}

static inline void SDA_Reset()
{
    LL_GPIO_ResetOutputPin(SDA_GPIOx, SDA_PIN);
}

static inline void SDA_Set()
{
    LL_GPIO_SetOutputPin(SDA_GPIOx, SDA_PIN);
}

static inline void SDA_SetDir(bool Output)
{
    LL_GPIO_SetPinMode(SDA_GPIOx, SDA_PIN, Output ? LL_GPIO_MODE_OUTPUT : LL_GPIO_MODE_INPUT);
}

static inline uint32_t SDA_ReadInput()
{
    return LL_GPIO_IsInputPinSet(SDA_GPIOx, SDA_PIN) ? 1 : 0;
}

  void AUDIO_AudioPathOff()
{
    LL_GPIO_ResetOutputPin(AUDIO_PA_GPIOx, AUDIO_PA_PIN);
}

  void AUDIO_AudioPathOn()
{
    LL_GPIO_SetOutputPin(AUDIO_PA_GPIOx, AUDIO_PA_PIN);
}

#define gTickMultiplier 48
...
static void SYSTEM_DelayMs(uint32_t Delay)
{
...
void BK4819_SetAGC(bool enable)
{
    ...
    BK4819_WriteRegister(BK4819_REG_7E, (regVal & ~(1 << 15) & ~(0b111 << 12)) | (!enable << 15) // 0  AGC fix mode
