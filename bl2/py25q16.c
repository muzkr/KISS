...
static void py25q16_read1(uint32_t Address, uint8_t *pBuffer, uint32_t Size)
{
#ifdef DEBUG
...
void py25q16_read(uint32_t addr, uint8_t *buf, uint32_t size)
{
    if (0 == size)
    {
        return;
    }
    py25q16_read1(addr, buf, size);
}

void py25q16_write(uint32_t Address, const uint8_t *pBuffer, uint32_t size)
{
    uint32_t SecIndex = Address / SECTOR_SIZE;
    uint32_t SecAddr = SecIndex * SECTOR_SIZE;
    uint32_t SecOffset = Address % SECTOR_SIZE;
    uint32_t SecSize = SECTOR_SIZE;
